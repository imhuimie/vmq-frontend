...
<script setup lang="ts">
  ...
  const breadcrumbItems = computed<BreadcrumbItem[]>(() => {
    ...
    // 过滤掉设置了hideInBread的路由
    const filteredMatched = matched.filter(item => !item.meta?.hideInBread)
    
    // 处理一级菜单和普通路由
    const isFirstLevel = filteredMatched[0]?.meta?.isFirstLevel
    const currentRoute = filteredMatched[filteredMatched.length - 1]

    // 如果过滤后没有路由，返回空数组
    if (!filteredMatched.length || !currentRoute) {
      return []
    }

    return isFirstLevel ? [createBreadcrumbItem(currentRoute)] : filteredMatched.map(createBreadcrumbItem)
  })
