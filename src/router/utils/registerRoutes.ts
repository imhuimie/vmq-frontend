...
function handleNormalRoute(
  ...
  if (component) {
    ...
    // 检查是否需要在主布局内显示
    if (converted.meta?.inLayout) {
      // 使用与handleLayoutRoute类似的处理方式
      converted.component = () => import('@/views/index/index.vue')
      converted.meta.isSubInLayout = true
      
      // 将原组件作为子路由
      const originalComponent = aliasComponent || loadComponent(component as string, routeName)
      const originalPath = converted.path
      const originalName = converted.name
      const originalMeta = { ...converted.meta }
      
      // 重置当前路由属性
      converted.name = `${String(originalName)}Layout`
      
      // 设置布局路由不在面包屑中显示，避免重复
      converted.meta.hideInBread = true
